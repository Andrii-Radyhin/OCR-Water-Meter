...
P.S. Because of not simple install of some [packages](https://detectron2.readthedocs.io/en/latest/tutorials/install.html) there is no .py files. Programm can be tested step by step with notebooks. .zip archive contains requirements.txt, but pipreqs can not detect detectron2 and it's may not work with pip also.
